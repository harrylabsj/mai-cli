...
class MaiCliMvpTest(unittest.TestCase):
    def run_cli(self, db_file, *args):
        output = StringIO()
        with redirect_stdout(output):
            mai.main(["--db", str(db_file), *args])
    ...
    def test_longjing_consultation_records_intent_without_order_or_payment_state(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payloads[1]["source_id"], "mai-cli-merchant-agent:seller-a")
    ...
    def test_cli_exposes_agent_lists_and_global_human_review_queue(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(agent["agent"]["id"], "mai-cli-merchant-agent:seller-a")
