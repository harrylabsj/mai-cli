...
TMP_DIR="$(mktemp -d)"
DB_FILE="$TMP_DIR/mai-cli.sqlite"
...
python3 "$ROOT_DIR/scripts/mai.py" --db "$DB_FILE" merchant create \
  --id seller-a \
  --name "West Lake Tea" \
  --city Hangzhou \
...
python3 "$ROOT_DIR/scripts/mai.py" --db "$DB_FILE" product add \
  --merchant seller-a \
  --sku tea-a \
...
python3 "$ROOT_DIR/scripts/mai.py" --db "$DB_FILE" buyer ask \
  --buyer alice \
...
python3 "$ROOT_DIR/scripts/mai.py" --db "$DB_FILE" agent run \
  --merchant seller-a \
  --once \
  --format json >"$TMP_DIR/agent.json"

python3 "$ROOT_DIR/scripts/mai.py" --db "$DB_FILE" buyer summarize \
  --conversation CONV-0001 \
  --format json >"$TMP_DIR/summary.json"

python3 "$ROOT_DIR/scripts/mai.py" --db "$DB_FILE" buyer intent \
  --conversation CONV-0001 \
...
assert 'display_name: "mai-cli"' in openai_yaml
assert "$mai-cli" in openai_yaml
